...
import {
  optimizeRoomLifecycle as runRoomLifecycleOptimization,
  type RoomLifecycleDesign,
  type RoomLifecycleOptimizationInput,
  type RoomLifecycleOptimizationResult,
} from '../lib/room_lifecycle_optimizer';
import {
  optimizeSiteGeometry as runSiteGeometryOptimization,
  type SiteGeometryOptimizationInput,
  type SiteGeometryOptimizationResult,
} from '../lib/site_geometry_optimizer';
import {
  calculateHeightAwareAirflow,
  optimizeHeightAwareAirflow,
  sampleHeightTemperature,
  type HeightAirflowProfile,
  type HeightAirflowSweepResult,
} from '../lib/height_airflow';
 ...
 *
 * Cross-sim inputs are visual and exploratory only. The thermal solver itself
 * is driven by room geometry, assemblies, climate, loads, vents and controls.
 *
 * Outputs:
...
export class V12HouseEngine {
  ...
  private _siteOrientationDeg: number = 90;
  private _siteSummerSolarAccess: number = 1;
  private _siteWinterSolarAccess: number = 1;
  private _lastSiteOptimization: SiteGeometryOptimizationResult | null = null;
  ...
  private _heightAirflowProfiles: HeightAirflowProfile[] = [];
  private _heightAirflowSweeps: HeightAirflowSweepResult[] = [];
   ...
   * Intake at floor level (z=0.15) on one wall, exhaust at ceiling
   * (z=ceiling-0.15) on the opposite wall.
  ...
  private _addFloorCeilingVents() {
    ...
    for (const room of this._rooms) {
      // Legacy plans stored every vent at z=0 because the old solver only had
      // floor-plan coordinates. Migrate those vents into an actual vertical
      // arrangement before deciding whether a pair already exists.
      for (const vent of room.vents) {
        if (vent.position.z > 0.001) continue;
        if (vent.type === 'intake') vent.position.z = Math.min(0.2, room.ceilingHeight * 0.08);
        else if (vent.type === 'exhaust') vent.position.z = Math.max(0.2, room.ceilingHeight - Math.min(0.2, room.ceilingHeight * 0.08));
        else vent.position.z = room.ceilingHeight * 0.5;
      }
      ...
      if (!hasIntake) {
        room.vents.push({
          ...
          position: new THREE.Vector3(
            room.x + 0.3,
            room.y + room.height * 0.2,
            Math.min(0.2, room.ceilingHeight * 0.08),
          ),
          flowRate,
          efficiency: 0,
      ...
      if (!hasExhaust) {
        room.vents.push({
          ...
          position: new THREE.Vector3(
            ...
            room.y + room.height * 0.8,
            Math.max(0.2, room.ceilingHeight - Math.min(0.2, room.ceilingHeight * 0.08)),
          ),
          flowRate,
          efficiency: 0,
          diameter: Math.min(250, Math.max(100, Math.round(roomVolume * 3))),
  ...
  update(optimizer: string, _mem: any, thermalMode: string) {
    ...
    // 0c. Resolve vertical temperature layers, vent height separation and
    // hydrostatic stack pressure before projecting the flow onto the plan.
    this._stepHeightAwareAirflow(effectiveMode);

    // 1. Plan-view Navier-Stokes solve. Gravity/stack flow is supplied by the
    // vertical model rather than pretending plan-Y is building height.
  ...
  private _stepHeightAwareAirflow(mode: string) {
    const sweepSignature = [
      mode,
      Math.round(this._outsideTemp / 2) * 2,
      ...this._rooms.map(room => [room.id, room.width, room.height, room.ceilingHeight, room.targetTemp, room.internalLoad].join(':')),
    ].join('|');
    if (sweepSignature !== this._heightAirflowSweepSignature) {
      this._heightAirflowSweepSignature = sweepSignature;
      this._heightAirflowSweepRuns += 1;
      this._heightAirflowSweeps = this._rooms.map(room => {
        const roomTempC = this._getAvgRoomTemp(room);
        const sweepMode: 'heating' | 'cooling' = mode === 'heating'
          ? 'heating'
          : mode === 'cooling' ? 'cooling' : roomTempC >= room.targetTemp ? 'cooling' : 'heating';
        const sweep = optimizeHeightAwareAirflow({
          roomId: room.id,
          roomName: room.name,
          mode: sweepMode,
          ceilingHeightM: room.ceilingHeight,
          floorAreaM2: room.width * room.height,
          roomTempC,
          targetTempC: room.targetTemp,
          outsideTempC: this._outsideTemp,
          hvacSupplyTempC: sweepMode === 'cooling' ? 14 : 35,
          internalHeatW: room.internalLoad,
          envelopeLoadW: room.width * room.height * 4.2 * Math.abs(this._outsideTemp - room.targetTemp),
        });
        const intake = room.vents.find(vent => vent.type === 'intake' || vent.type === 'heat_recovery');
        const exhaust = room.vents.find(vent => vent.type === 'exhaust');
        for (const vent of [intake, exhaust]) {
          if (!vent) continue;
          vent.flowRate = sweep.best.designFlowM3s;
          vent.diameter = sweep.best.ventDiameterM * 1000;
          vent.powered = sweep.best.powered;
        }
        if (intake) intake.position.z = sweep.best.intakeHeightM;
        if (exhaust) exhaust.position.z = sweep.best.exhaustHeightM;
        return sweep;
      });
      // The old equilibrium mutator changed one random property and never
      // restored the best state. A full deterministic sweep now owns vent
      // selection, so keep that legacy mutator disabled.
      this._ventOptCycle = 20;
    }

    this._heightAirflowProfiles = this._rooms.map(room => {
      const intake = room.vents.find(vent => vent.type === 'intake' || vent.type === 'heat_recovery');
      const exhaust = room.vents.find(vent => vent.type === 'exhaust');
      const designFlowM3s = Math.max(intake?.flowRate || 0, exhaust?.flowRate || 0, 0.001);
      const profile = calculateHeightAwareAirflow({
        roomId: room.id,
        roomName: room.name,
        ceilingHeightM: room.ceilingHeight,
        floorAreaM2: room.width * room.height,
        roomTempC: this._getAvgRoomTemp(room),
        outsideTempC: this._outsideTemp,
        supplyTempC: mode === 'cooling' ? 14 : mode === 'heating' ? 35 : this._outsideTemp,
        intakeHeightM: intake?.position.z ?? Math.min(0.2, room.ceilingHeight * 0.08),
        exhaustHeightM: exhaust?.position.z ?? Math.max(0.2, room.ceilingHeight - 0.2),
        designFlowM3s,
        ventDiameterM: Math.max(intake?.diameter || 0, exhaust?.diameter || 0, 150) / 1000,
        powered: Boolean(intake?.powered || exhaust?.powered),
        internalHeatW: room.internalLoad,
      });
      const ventAreaM2 = Math.PI * (Math.max(intake?.diameter || exhaust?.diameter || 150, 40) / 2000) ** 2;
      for (const vent of room.vents) {
        if (vent.type !== 'intake' && vent.type !== 'exhaust' && vent.type !== 'heat_recovery') continue;
        vent.currentFlowRate = profile.effectiveFlowM3s;
        vent.airSpeed = profile.effectiveFlowM3s / Math.max(0.0001, ventAreaM2);
        vent.currentSupplyTemp = mode === 'cooling' ? 14 : mode === 'heating' ? 35 : this._outsideTemp;
        vent.currentExhaustTemp = sampleHeightTemperature(profile, vent.position.z, this._getAvgRoomTemp(room));
      }
      return profile;
    });
  }
  ...
  private _stepNavierStokes() {
    ...
    for (let sub = 0; sub < this._nsSubsteps; sub++) {
      ...
      for (const room of this._rooms) {
        ...
        const heightProfile = this._heightAirflowProfiles.find(profile => profile.roomId === room.id);
        for (const vent of room.vents) {
          ...
          const reverseNaturalFlow = Boolean(heightProfile?.reverseStackRisk && !vent.powered);
          const isIntake = reverseNaturalFlow ? vent.type === 'exhaust' : vent.type === 'intake';
          const flowRate = (vent.currentFlowRate ?? vent.flowRate ?? 0.02)
            * (heightProfile?.displacementEffectiveness ?? 1);
      ...
      // ── Step 2: plan pressure from horizontal thermal gradients ──
      // Gravity is resolved by _stepHeightAwareAirflow. The grid axes here
      // are both horizontal floor-plan axes, so direct buoyancy on plan-Y was
    ...
    if (this._ventOptCooldown > 0) {
      ...
      for (const room of this._rooms) {
        ...
        const heightProfile = this._heightAirflowProfiles.find(profile => profile.roomId === room.id);
        if (heightProfile) {
          score += Math.abs(heightProfile.stratificationC) * 0.15;
          if (heightProfile.shortCircuitRisk) score += 2;
          if (heightProfile.reverseStackRisk) score += 1;
        }
      }
      ...
      if (allVents.length > 0) {
        ...
        if (action < 0.35) {
          ...
          // and ceiling while exploring displacement and stack ventilation.
          const room = this._rooms.find(candidate => candidate.vents.includes(vent));
  ...
  private _calcSolarGain(room: RoomDef, climate: typeof CLIMATE_DATA[string]): number {
    ...
    for (const win of room.windows) {
      ...
      const month = Math.floor(this._annualClimate.simulatedDayOfYear / 30.4375) + 1;
      const southern = climate.lat < 0;
      const summer = southern ? [12, 1, 2].includes(month) : [6, 7, 8].includes(month);
      const winter = southern ? [6, 7, 8].includes(month) : [12, 1, 2].includes(month);
      const siteShadeModifier = summer
        ? this._siteSummerSolarAccess
        : winter ? this._siteWinterSolarAccess : (this._siteSummerSolarAccess + this._siteWinterSolarAccess) / 2;
      gain += climate.solarIrradiance * win.shgc * win.area * orientFactor * siteShadeModifier;
    }
    return gain;
  }
  ...
  private _stepVentReadings(mode: string) {
    for (const room of this._rooms) {
      ...
      for (const vent of room.vents) {
        ...
        const idx = gy * this._gridW + gx;
        const planTemp = this._thermalGrid[idx] || 22;
        const localTemp = sampleHeightTemperature(heightProfile, vent.position.z, planTemp);
        ...
        const currentFlowRate = vent.currentFlowRate ?? vent.flowRate ?? area * planarSpeed;
        const speed = currentFlowRate / Math.max(area, 0.0001);
        const massFlowRate = 1.2 * currentFlowRate;
    ...
    return {
      ...
      vents: vents.map((v, i) => ({
        ...
        position: new THREE.Vector3(
          v.x,
          v.y,
          v.type === 'intake' ? Math.min(0.2, ceilingH * 0.08)
            : v.type === 'exhaust' ? Math.max(0.2, ceilingH - Math.min(0.2, ceilingH * 0.08))
              : ceilingH * 0.5,
        ),
        flowRate: v.flow,
        efficiency: v.eff || 0,
  ...
  setVentHeight(roomId: string, type: 'intake' | 'exhaust', heightM: number) {
    const room = this._rooms.find(candidate => candidate.id === roomId);
    const vent = room?.vents.find(candidate => candidate.type === type);
    if (!room || !vent) return null;
    vent.position.z = Math.max(0.08, Math.min(room.ceilingHeight - 0.08, Number(heightM)));
    this._equilibriumFound = false;
    this._equilibriumScore = 0;
    this._turbulenceHistory.length = 0;
    this._ventOptCooldown = 45;
    this._stepHeightAwareAirflow(this._autoThermalMode);
    return this._heightAirflowProfiles.find(profile => profile.roomId === roomId) || null;
  }

  runHeightAirflowSweep(roomId?: string) {
    this._heightAirflowSweepSignature = '';
    this._stepHeightAwareAirflow(this._autoThermalMode);
    return roomId
      ? this._heightAirflowSweeps.find(sweep => sweep.roomId === roomId) || null
      : this._heightAirflowSweeps;
  }

  optimizeSiteGeometry(input: SiteGeometryOptimizationInput = {}): SiteGeometryOptimizationResult {
    const result = runSiteGeometryOptimization(input);
    this._lastSiteOptimization = result;
    return result;
  }

  /**
   * Replace the V12 floorplan with the optimizer's non-overlapping room layout.
   * The plan remains axis-aligned in the 2D solver while orientation and seasonal
   * self-shading modifiers are retained for the solar heat-gain calculation.
   */
  applySiteGeometryOptimization(result: SiteGeometryOptimizationResult) {
    if (!result.improvement.qualifiesAsImprovement || !result.best.feasible) {
      throw new Error('Only a feasible, net-positive site design can be applied.');
    }

    const design = result.best.design;
    const southern = result.location.latitudeDeg < 0;
    const equatorOrientation: WindowSpec['orientation'] = southern ? 'N' : 'S';
    const absoluteLatitude = Math.abs(result.location.latitudeDeg);
    this._globeZone = absoluteLatitude < 23.5 ? 'tropical' : absoluteLatitude >= 48 ? 'cold' : 'temperate';
    this._zoneConstraints = ZONE_CONSTRAINTS[this._globeZone];
    this._zoneConfigured = true;
    this._siteOrientationDeg = design.orientationDeg;
    this._siteSummerSolarAccess = clamp01(1 - result.best.solar.summerShadePercent / 100);
    this._siteWinterSolarAccess = clamp01(result.best.solar.winterSolarAccessPercent / 100);
    this._lastSiteOptimization = result;
    this._indoorTarget = result.best.rooms[0]?.targetTempC || 22;

    const locationName = result.location.name.toLowerCase();
    if (locationName.includes('darwin')) this._climate = 'tropical_darwin';
    else if (locationName.includes('brisbane')) this._climate = 'subtropical_brisbane';
    else if (locationName.includes('melbourne') || locationName.includes('bendigo')) this._climate = 'cool_temperate_melb';
    else if (locationName.includes('alice')) this._climate = 'arid_alice';
    else if (this._globeZone === 'cold') this._climate = 'alpine';
    else this._climate = 'temperate_sydney';

    const preferredWall = this._zoneConstraints.preferredWallTypes.find(key => WALL_LIBRARY[key]) || 'brick_veneer';
    const glazingType = this._globeZone === 'cold' ? 'triple_lowE' : 'double_lowE';
    const glazing = WINDOW_TYPES[glazingType];
    this._rooms = result.best.rooms.map((layout, index) => {
      const equatorWindowArea = Math.max(0.6,
        layout.width * design.ceilingHeightM * design.equatorGlazingRatio * 0.72);
      const sideOrientation: WindowSpec['orientation'] = index % 2 === 0 ? 'E' : 'W';
      return {
        id: `site_${layout.id}`,
        name: layout.name,
        x: layout.x,
        ...
        width: layout.width,
        height: layout.depth,
        ceilingHeight: design.ceilingHeightM,
        wallType: preferredWall,
        windows: [
          { type: glazingType, uValue: glazing.uValue, shgc: glazing.shgc, area: equatorWindowArea, orientation: equatorOrientation },
          { type: glazingType, uValue: glazing.uValue, shgc: glazing.shgc, area: equatorWindowArea * design.otherGlazingRatio / Math.max(design.equatorGlazingRatio, 0.05) * 0.45, orientation: sideOrientation },
        ],
        vents: [],
        internalLoad: layout.internalLoadW,
        targetTemp: layout.targetTempC,
        hasEsky: layout.role === 'living',
        hasStubby: layout.role === 'kitchen',
      } satisfies RoomDef;
    });
  ...
  getSiteDesignState() {
    return {
      orientationDeg: this._siteOrientationDeg,
      summerSolarAccess: this._siteSummerSolarAccess,
      winterSolarAccess: this._siteWinterSolarAccess,
      lastOptimization: this._lastSiteOptimization,
    };
  }

  /**
   * Search room geometry and supply/return vent arrangements against a combined
   * operational + manufacturing-energy objective. The search is deterministic
   * for a given seed, so designs are reproducible and testable.
   */
  optimizeRoomLifecycle(
    roomId: string,
    overrides: RoomLifecycleOptimizationInput = {},
  ): RoomLifecycleOptimizationResult {
    const room = this._rooms.find(r => r.id === roomId) || this._rooms[0];
    if (!room) throw new Error('No room is available to optimize.');

    const climate = CLIMATE_DATA[this._climate] || CLIMATE_DATA.temperate_sydney;
    const mode = overrides.mode || 'cooling';
    const intake = room.vents.find(v => v.type === 'intake');
    const exhaust = room.vents.find(v => v.type === 'exhaust');
    const wall = WALL_LIBRARY[room.wallType] || WALL_LIBRARY.brick_veneer;
    const floorAreaM2 = room.width * room.height;
    const windowAreaM2 = room.windows.reduce((sum, window) => sum + window.area, 0);
    const windowUValue = windowAreaM2 > 0
      ? room.windows.reduce((sum, window) => sum + window.uValue * window.area, 0) / windowAreaM2
      : 1.8;
    const normaliseVent = (vent: VentSpec | undefined, fallback: { x: number; y: number; z: number }) => vent
      ? {
          x: (vent.position.x - room.x) / Math.max(room.width, 0.1),
          y: (vent.position.y - room.y) / Math.max(room.height, 0.1),
          z: vent.position.z / Math.max(room.ceilingHeight, 0.1),
        }
      : fallback;
    const baselineFlow = Math.max(intake?.flowRate || 0, exhaust?.flowRate || 0, 0.01);
    const baselineDiameterM = Math.max(intake?.diameter || exhaust?.diameter || 150, 50) / 1000;
    const baselineStrategy = intake && exhaust
      && Math.abs(intake.position.z - exhaust.position.z) > room.ceilingHeight * 0.45
      ? 'stack' as const
      : 'cross' as const;

    return runRoomLifecycleOptimization({
      roomName: room.name,
      mode,
      targetTempC: room.targetTemp,
      outdoorDesignTempC: mode === 'cooling' ? climate.summerDesign : climate.winterDesign,
      minFloorAreaM2: Math.max(6, floorAreaM2 * 0.7),
      maxFloorAreaM2: Math.max(floorAreaM2 * 1.2, floorAreaM2 + 3),
      minWidthM: Math.max(1.8, room.width * 0.65),
      maxWidthM: Math.max(room.width * 1.25, room.width + 1),
      minLengthM: Math.max(1.8, room.height * 0.65),
      maxLengthM: Math.max(room.height * 1.25, room.height + 1),
      minCeilingHeightM: Math.max(2.1, room.ceilingHeight * 0.88),
      maxCeilingHeightM: Math.max(room.ceilingHeight * 1.15, room.ceilingHeight + 0.25),
      wallRValue: wall.totalR,
      windowAreaM2,
      windowUValue,
      internalGainsW: room.internalLoad,
      solarGainsW: windowAreaM2 * climate.solarIrradiance * 0.22,
      conditioningHoursPerYear: mode === 'cooling'
        ? Math.max(600, climate.coolingDegreeDays * 0.9)
        : Math.max(600, climate.heatingDegreeDays * 0.9),
      baseline: {
        widthM: room.width,
        lengthM: room.height,
        ceilingHeightM: room.ceilingHeight,
        strategy: baselineStrategy,
        intake: normaliseVent(intake, { x: 0.05, y: 0.2, z: 0.15 }),
        exhaust: normaliseVent(exhaust, { x: 0.95, y: 0.8, z: 0.85 }),
        flowRateM3s: baselineFlow,
        ventDiameterM: baselineDiameterM,
      },
      ...overrides,
    });
  }

  /** Apply an optimizer result to the live V12 room and rebuild its solver grids. */
  applyRoomLifecycleDesign(roomId: string, design: RoomLifecycleDesign, targetTempC?: number) {
    ...
    const oldBottom = room.y + room.height;
    const widthDelta = design.widthM - room.width;
    const heightDelta = design.lengthM - room.height;

    // Keep adjacent rooms attached to the edited floorplan instead of leaving
    // gaps or placing the resized room directly over its neighbours.
    for (const other of this._rooms) {
      if (other.id === room.id) continue;
      const shiftX = other.x >= oldRight - 0.01 ? widthDelta : 0;
      const shiftY = other.y >= oldBottom - 0.01 ? heightDelta : 0;
      if (shiftX === 0 && shiftY === 0) continue;
      other.x += shiftX;
      other.y += shiftY;
      for (const vent of other.vents) {
        vent.position.x += shiftX;
        vent.position.y += shiftY;
      }
    }

    room.width = design.widthM;
    room.height = design.lengthM;
    room.ceilingHeight = design.ceilingHeightM;
    if (targetTempC !== undefined) room.targetTemp = targetTempC;

    const retainedVents = room.vents.filter(v => v.type !== 'intake' && v.type !== 'exhaust');
    const createVent = (type: 'intake' | 'exhaust', position: RoomLifecycleDesign['intake']): VentSpec => ({
      id: `${room.id}_optimized_${type}`,
      type,
      position: new THREE.Vector3(
        room.x + position.x * room.width,
        room.y + position.y * room.height,
        position.z * room.ceilingHeight,
      ),
      flowRate: design.flowRateM3s,
      efficiency: 0,
      diameter: Math.round(design.ventDiameterM * 1000),
      powered: true,
      currentFlowRate: 0,
      currentSupplyTemp: room.targetTemp,
      currentExhaustTemp: room.targetTemp,
      airSpeed: 0,
    });
    room.vents = [
      ...retainedVents,
      createVent('intake', design.intake),
      createVent('exhaust', design.exhaust),
    ];
  ...
  getThermalGrid() {
    return {
      ...
      heightProfiles: this._heightAirflowProfiles,
      heightSweeps: this._heightAirflowSweeps,
      heightSweepRuns: this._heightAirflowSweepRuns,
