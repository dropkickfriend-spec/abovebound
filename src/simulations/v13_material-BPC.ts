...
export class V13MaterialEngine {
  ...
  /**
   * REQUEST FROM COOLING SIM: "I need a wall material with these thermal properties"
   * V13 searches its element database, builds candidate composites, and returns the best.
   * Starts from hydrogen (Z=1) and builds up — elements refer to themselves.
   */
  requestCompositeForThermal(requirements: {
    targetConductivity?: number;   // W/mK — lower = better insulator
    targetDensity?: number;        // kg/m³ — lower = lighter
    targetMeltingPoint?: number;   // K — higher = more heat resistant
    maxCost?: number;              // relative 0-10
    purpose: 'insulation' | 'heat_sink' | 'phase_change' | 'radiation_shield' | 'structural';
  }): MaterialComposite | null {
    // Strategy: start from hydrogen (1), scan elements, find best candidates for purpose
    const candidates: { z: number; score: number }[] = [];

    // Score every element we know about for this purpose
    for (const el of Object.values(BY_Z)) {
      if (!el) continue;
      let score = 0;
      const k = el.conductivity || 0.01;
      const mp = el.meltingPoint || 300;
      const d = el.density || 1;

      switch (requirements.purpose) {
        case 'insulation':
          // Low conductivity, low density, high melting point = good insulator
          score = (1 / (k + 0.001)) * 0.5 + (1 / (d + 1)) * 0.3 + (mp / 3000) * 0.2;
          break;
        case 'heat_sink':
          // High conductivity, high density, high specific heat
          score = k * 0.5 + d * 0.0001 + (el.specificHeat || 500) * 0.001;
          break;
        case 'phase_change':
          // Melting point near room temp range (280-340K), high specific heat
          score = mp > 250 && mp < 400 ? 5 : 1;
          score += (el.specificHeat || 500) * 0.002;
          break;
        case 'radiation_shield':
          // High Z (more electrons = better radiation absorption), high density
          score = el.atomicNumber * 0.1 + d * 0.0001;
          break;
        case 'structural':
          // High melting point, moderate density, balance of properties
          score = (mp / 2000) * 0.4 + (d > 1000 && d < 8000 ? 3 : 1) * 0.3 + (el.hardness || 3) * 0.3;
          break;
      }

      if (requirements.targetConductivity !== undefined) {
        // Penalize deviation from target
        score -= Math.abs(k - requirements.targetConductivity) * 0.1;
      }
      if (requirements.targetDensity !== undefined) {
        score -= Math.abs(d - requirements.targetDensity) * 0.0001;
      }

      candidates.push({ z: el.atomicNumber, score });
    }

    // Sort by score, take top 3-5 elements
    candidates.sort((a, b) => b.score - a.score);
    const top = candidates.slice(0, Math.min(5, candidates.length));
    if (top.length === 0) return null;

    // Build composite from top candidates with weighted fractions
    const totalScore = top.reduce((s, c) => s + Math.max(0, c.score), 0) || 1;
    const elements = top.map(c => ({
      z: c.z,
      fraction: Math.max(0.05, c.score / totalScore),
    }));

    // Always include hydrogen as a reference point (Brandon: "start hydrogen off by saying maybe 1")
    if (!elements.find(e => e.z === 1)) {
      elements.push({ z: 1, fraction: 0.05 });
    }

    try {
      const composite = this.buildComposite(elements);
      console.log(`[V13] Built ${requirements.purpose} composite: ${elements.map(e => `Z${e.z}:${(e.fraction*100).toFixed(0)}%`).join(' + ')}`);
      return composite;
    } catch {
      return null;
    }
